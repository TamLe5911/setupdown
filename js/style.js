$(function () {
    let header = function () {
        $(".method-dropdown-item").on("click", function (e) {
            $(this).toggleClass("active");
            $(this).closest(".method-dropdown-link").find(".method-dropdown-content").toggleClass("show");
        });
        $('#toggleMobileMenu').on("click", function () {
            $('.nav-menu').addClass('show');
            $('.header-top-content .right-header').addClass('show');
        });
        $('#closeMenuMobile').on("click", function () {
            $('.nav-menu').removeClass('show');
            $('.header-top-content .right-header').removeClass('show');
        });
        $('.close-popup-mobile').on("click", function () {
            $(this).closest('.method-dropdown-content').removeClass('show');
        });
        document.addEventListener("click", function (e) {
            let alls = document.querySelectorAll(".method-dropdown-item.active");
            for (let it of alls) {
                if (!it.closest(".method-dropdown-item").contains(e.target) && !$(it).siblings().get(0).contains(e.target)) {
                    console.log(123);
                    it.classList.remove("active");
                    it.closest(".method-dropdown-link").querySelector(".method-dropdown-content").classList.remove("show");
                }
            }
        });
        if ($(window).width() > 992){
